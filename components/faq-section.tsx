...
const FAQ: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: "Около 70 км на одной батарее (60V/33Ah) — подтверждено отзывами наших арендаторов, не маркетинговое обещание. В комплекте две таких батареи — итого до 140 км на смену. Реальный запас зависит от веса курьера, стиля езды и температуры.",
  },
  {
    q: "Что если обе батареи разрядятся?",
    a: "До 140 км на смену — это с запасом под типичную курьерскую загрузку. Если всё-таки разрядилось — велосипед едет и без мотора как обычный, доедешь до дома или до точки. Но на практике до этого не доходит: вторую батарею курьеры обычно трогают только к вечеру.",
  },
  {
  ...
  },
  {
    ...
    a: "Сейчас работаем только с гражданами РФ — нужен российский паспорт. ВНЖ, РВП, иностранный паспорт и патент пока не принимаем. Если ситуация изменится — обновим этот ответ.",
  },
  {
    q: "А по ПДД можно ехать 70 км/ч? Нужны ли права?",
    a: "По стоковой прошивке велосипед едет до 25 км/ч — это категория «электровелосипед», без прав, без регистрации, можно по велодорожкам. Технически мотор 2000 Вт способен на большее (до 70 км/ч), но в этом режиме ТС юридически переходит в категорию «мопед»: нужны права категории «М» (или любой другой), движение только по проезжей части, велодорожки запрещены. Что выбрать — твоё решение и твоя ответственность.",
  },
];
