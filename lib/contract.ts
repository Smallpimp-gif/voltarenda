...
export async function generateContractBlob(data: ContractData): Promise<Blob> {
  ...
  const children: Paragraph[] = [
    ...
    p(
      `2.1. Арендная плата составляет ${rub.format(WEEKLY)} ₽ в неделю. Срок аренды — ${WEEKS} (сорок одна) недель; общая сумма за срок аренды — ${rub.format(TOTAL)} ₽. Платёж вносится авансом не позднее 17:00 первого дня недельного периода наличными или переводом на счёт/карту Арендодателя; подтверждение — чек, расписка или электронная квитанция.`,
    ),
    p(
